...
TARGET="${1:-balanced}"
case "${TARGET}" in
  core|fast|balanced|full|gpt-oss) ;;
  *)
    echo "Usage: ./scripts/install-models.sh [core|fast|balanced|full|gpt-oss]" >&2
    ...
    echo "  full: balanced plus EchoMimicV3 and GPT-OSS 20B" >&2
    echo "  gpt-oss: GPT-OSS 20B environment and weights only" >&2
    exit 2
    ;;
esac
...
if [[ "${TARGET}" == "gpt-oss" ]]; then
  "${ROOT_DIR}/scripts/install-gpt-oss-20b.sh"
